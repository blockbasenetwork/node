    ...
    public class ApiWebHostBuilder
    {
        ...
        public ApiWebHostBuilder ConfigureMainSettings(string[] args)
        {
            ...
            _webHostBuider.ConfigureServices((hostContext, services) =>
            {
                ...
                {
                    c.SwaggerDoc("providerApi", new OpenApiInfo { Title = "Service Provider API", Version = "v0.1", Description = $"Node Version: {Assembly.GetEntryAssembly().GetName().Version.ToString()} | Node Account: {node}" });
                    c.SwaggerDoc("requesterApi", new OpenApiInfo { Title = "Service Requester API", Version = "v0.1", Description = $"Node Version: {Assembly.GetEntryAssembly().GetName().Version.ToString()} | Node Account: {node}" });
                    c.SwaggerDoc("networkApi", new OpenApiInfo { Title = "Network API", Version = "v0.1", Description = $"Node Version: {Assembly.GetEntryAssembly().GetName().Version.ToString()} | Node Account: {node}" });
