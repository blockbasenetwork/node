...
namespace BlockBase.Runtime.Provider.StateMachine.SidechainState
{
    public class BlockProductionStateManager : AbstractStateManager<StartState, EndState>
    {
        ...
        protected override IState BuildState(string state)
        {
            ...
            if(state == typeof(SynchronizeNodeState).Name) return new SynchronizeNodeState(_logger, _mainchainService, _mongoDbProducerService, _sidechainPool, _nodeConfigurations, _networkConfigurations, _networkService, _transactionValidationsHandler, _peerConnectionsHandler);
