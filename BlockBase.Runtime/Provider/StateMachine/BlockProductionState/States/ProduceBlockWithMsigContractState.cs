...
namespace BlockBase.Runtime.Provider.StateMachine.BlockProductionState.States
{
    ...
    public class ProduceBlockWithMsigContractState : ProviderAbstractState<StartState, EndState, WaitForEndConfirmationState>
    {
        ...
        private bool _hasProposedBlock;
        private bool _hasApprovedBlock;
        private bool _hasEnoughApprovals;
        private bool _hasBlockBeenVerified;
        private string _proposalHash;
        private DateTime _lastTimeTriedToSendBlock;
        ...
        public ProduceBlockWithMsigContractState(ILogger logger, IMainchainService mainchainService,
            ...
            NodeConfigurations nodeConfigurations, NetworkConfigurations networkConfigurations, BlockRequestsHandler blockSender) : base(logger, sidechainPool, mainchainService)
        {
            ...
            _hasProposedBlock = false;
            _hasApprovedBlock = false;
            _hasEnoughApprovals = false;
        ...
        protected override async Task DoWork()
        {
            ...
            if (_hasProviderBuiltNewBlock && !_hasProposedBlock)
                await TryProposeTransaction(_blockHash);

            if (_hasProviderBuiltNewBlock && _hasProposedBlock && !_hasApprovedBlock)
                await TryApproveTransaction();

            if (_hasProviderBuiltNewBlock && _hasProposedBlock && _hasApprovedBlock && !_hasEnoughApprovals && _lastTimeTriedToSendBlock.AddSeconds(15) <= DateTime.UtcNow)
            {
                ...
                _lastTimeTriedToSendBlock = DateTime.UtcNow;
            }

            if (_hasProviderBuiltNewBlock && _hasProposedBlock && _hasApprovedBlock && _hasEnoughApprovals && !_hasBlockBeenVerified)
            {
                await TryExecuteTransaction();
            }
        }
        ...
        protected override async Task UpdateStatus()
        {
            ...
            if (_hasProposedBlock && string.IsNullOrEmpty(_proposalHash))
            {
                var proposal = await _mainchainService.RetrieveProposal(_nodeConfigurations.AccountName, _sidechainPool.ClientAccountName);
                _proposalHash = proposal.TransactionHash;
            }

            var approvals = await _mainchainService.RetrieveApprovals(_nodeConfigurations.AccountName, _sidechainPool.ClientAccountName);
            _hasApprovedBlock = approvals.ProvidedApprovals.Any(t => t.PermissionLevel.actor == _nodeConfigurations.AccountName);
            if (_hasProviderBuiltNewBlock && _hasApprovedBlock && !_hasEnoughApprovals)
            {
                _hasEnoughApprovals = approvals.ProvidedApprovals.Count > approvals.RequestedApprovals.Count;
            }
        }
        ...
        private async Task TryProposeTransaction(string blockHash)
        {
            await _mainchainService.ProposeBlockVerification(_sidechainPool.ClientAccountName, _nodeConfigurations.AccountName, _producerList.Select(p => p.Key).ToList(), blockHash);
        }

        private async Task TryApproveTransaction()
        {
            await _mainchainService.ApproveTransaction(_nodeConfigurations.AccountName, _sidechainPool.ClientAccountName, _nodeConfigurations.AccountName, _proposalHash);
        }

        private async Task TryExecuteTransaction()
        {
            await _mainchainService.ExecuteTransaction(_nodeConfigurations.AccountName, _sidechainPool.ClientAccountName, _nodeConfigurations.AccountName);
